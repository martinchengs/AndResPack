...
#应用名称
apk_name=config.get("Build-Config","apk.name")
#应用版本名称
apk_version=config.get("Build-Config","apk.version")
...
# 创建生成目录,与文件名相关
output_dir = 'apks_' + apk_name +'_v'+ apk_version + '/'
# 目录不存在则创建
...
for line in channel_array:
    ...
    # 拼接对应渠道号的apk
    target_apk = output_dir + apk_name +"_v"+apk_version + "_" + target_channel + src_apk_extension
        # 拷贝建立新apk
